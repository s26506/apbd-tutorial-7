...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<object> result = Emps.Select(emp => new {Praca = emp.Job, LiczbaPracownikow = Emps.Count()});
            return result;
        }
        ...
        public static IEnumerable<object> Task10()
        {
            ...
            var defaultData = new[] { new { Ename = "Brak wartości", Job = (string)null, HireDate = (DateTime?)null } };
            IEnumerable<object> result = empData.Union(defaultData);
            return result;
        }
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result = Emps.GroupBy(emp => emp.Deptno)
                .Where(g => g.Count() > 1)
                .Select(g => new { name = Depts.FirstOrDefault(d => d.Deptno == g.Key).Dname, numOfEmployees = g.Count() })
                .ToList();
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = Depts.Where(d => Emps.Count(emp => emp.Deptno == d.Deptno) == 5 || !Emps.Any(emp => emp.Deptno == d.Deptno))
                .OrderBy(d => d.Dname);
            return result;
        }
